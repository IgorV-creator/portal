...
let UserService = class UserService {
    constructor(userRepository, roleService) {
        ...
        this.addRole = async (dto) => {
            ...
            throw new common_2.HttpException('Пользователь или роль не найдены', common_1.HttpStatus.NOT_FOUND);
        };
        this.ban = async (dto) => {
            const user = await this.userRepository.findByPk(dto.userId);
            if (!user) {
                throw new common_2.HttpException('Пользователь не найден', common_1.HttpStatus.NOT_FOUND);
            }
            user.banned = true;
